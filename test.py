...
def generate_image_from_text(text, queue):
    try:
...
def process_images(df):
    for index, row in df.iterrows():
        text = row["Titre"]
        queue = Queue()
        process = Process(target=generate_image_from_text, args=(text, queue))
        process.start()
        process.join()
        image = queue.get()
        if image:
            ...
            print(f"Failed to generate image for index {index}")

    df.to_csv("choix_tendances_tags_with_images.csv", sep=";", index=False)
    print("Images generated and added to the CSV successfully!")
...
def main():
    ...
    if choix == "1":
        ...
        start = input("\n\nEntrez au format JJ/MM/AAAA la timeframe que vous souhaitez appliquer à votre recherche\nATTENTION ! Tous les résultats seront issus d'articles parus dans cet intervalle de temps\nDate de début : ")
